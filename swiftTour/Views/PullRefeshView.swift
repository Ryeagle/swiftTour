...
class PullRefeshView: UIView {
    ...
    struct Constants {
        struct Images {
            static let pullRefreshViewBackgroud = "pull-refresh-view-background"
    ...
    init(frame: CGRect, scrollView: UIScrollView) {
        ...
        let pullRefreshViewBackgroundView = UIImageView(image: UIImage(named: Constants.Images.pullRefreshViewBackgroud))
        pullRefreshViewBackgroundView.frame = bounds
        pullRefreshViewBackgroundView.contentMode = .scaleAspectFill
        pullRefreshViewBackgroundView.clipsToBounds = true
        addSubview(pullRefreshViewBackgroundView)
        
        // Create custom paths for flying-saucer to animate to.
        paths = customPaths(frame: CGRect(x: 0.0, y: self.bounds.height / 5, width: self.bounds.width / 1.8, height: self.bounds.height / 1.5))
    ...
    func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        if !isRefreshing && self.self.progress >= 1.0 {
    ...
    func beginRefreshing() {
        ...
        hoverAnimation.toValue = NSValue(cgPoint: CGPoint(x: 0.0, y: 10.0))
        hoverAnimation.autoreverses = true
        hoverAnimation.duration = 0.2
        hoverAnimation.repeatCount = FLT_MAX
        ...
        flyingSaucerLayer.add(hoverAnimation, forKey: "hoverAnimation")
        let enterPath = paths[0]
    ...
    func endRefreshing() {
        ...
        flyingSaucerLayer.removeAnimation(forKey: "hoverAnimation")
        ...
        exitPathAnimation.timingFunctions = [CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseInEaseOut)]
